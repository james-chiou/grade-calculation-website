...
function setGPA() {
  ...
  if (creditSum == 0) {
    ...
    res = (sum / creditSum).toFixed(2);
  }
  document.getElementById("result-gpa").innerText = res;
}
...
allTrash.forEach((trash) => {
  trash.addEventListener("click", (e) => {
    ...
    //e.target.parentElement.parentElement.remove(); // 不會有動畫，而是直接移除
  });
});
// 因為JS執行速度太快，所以不會執行CSS中的動畫特效設定
allTrash.forEach((trash) => {
...
function handleSorting(direction) {
  ...
  for (let i = 0; i < graders.length; i++) {
    ...
    if (
      ...
      array.push(class_object);
    }
  }

  //取得array後，我們可以把成績string換成數字
  for (let i = 0; i < array.length; i++) {
    array[i].class_grade_number = convertor(array[i].class_grade);
  }
  ...
  for (let i = 0; i < array.length; i++) {
    ...
    <div class="grade">
        ...
        class="class-type"
        list="opt"
        value="${array[i].class_name}"
        /><!--
        --><input
        type="text"
        placeholder="class number"
        class="class-number"
        value="${array[i].class_number}"
        /><!--
        --><input
        type="number"
        placeholder="credits"
  ...
  for (let i = 0; i < graders.length; i++) {
    graders[i].children[3].value = array[i].class_grade;
  }
